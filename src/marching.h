...
const std::array<std::array<int, 2>, 12> pairTable = {{
    {0, 1}, {1, 2}, {2, 3}, {3, 0},
    {4, 5}, {5, 6}, {6, 7}, {7, 4},
    {0, 4}, {1, 5}, {2, 6}, {3, 7},
}};
...
const std::array<std::vector<int>, 256> triangleTable = {{
    {},
    {3, 8, 0},
    {9, 1, 0},
    {3, 8, 1, 1, 8, 9},
    ...
    std::vector<vec3> &out)
{
    ...
    for (int i = 0; i < 12; i++) {
        const int bit = 1 << i;
        if (edgeTable[mask] & bit) {
            const int a = pairTable[i][0];
            const int b = pairTable[i][1];
            const real t = (x - v[a]) / (v[b] - v[a]);
            points[i] = p[a] + (p[b] - p[a]) * t;
        }
    }
